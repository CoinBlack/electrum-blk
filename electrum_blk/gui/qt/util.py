...
from electrum_blk.util import (FileImportFailed, FileExportFailed, resource_path, EventListener, event_listener,
                           get_logger, UserCancelled, UserFacingException)
from electrum_blk.invoices import (PR_UNPAID, PR_PAID, PR_EXPIRED, PR_INFLIGHT, PR_UNKNOWN, PR_FAILED, PR_ROUTING,
                               PR_UNCONFIRMED, PR_BROADCASTING, PR_BROADCAST)
from electrum_blk.logging import Logger
from electrum_blk.qrreader import MissingQrDetectionLib, QrCodeResult
...
def export_meta_gui(electrum_window: 'ElectrumWindow', title, exporter):
    ...
    filename = getSaveFileName(
        ...
        filename='electrum-blk_{}.json'.format(title),
        filter=filter_,
        config=electrum_window.config,
    )
    if not filename:
