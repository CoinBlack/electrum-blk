...
from electrum_blk.invoices import (PR_DEFAULT_EXPIRATION_WHEN_CREATING,
...
from electrum_blk.util import (parse_URI, InvalidBitcoinURI, TxMinedInfo, maybe_extract_lightning_payment_identifier,
...
from electrum_blk.lnurl import decode_lnurl, request_lnurl, callback_lnurl, LNURLError, LNURL6Data
...
class SendScreen(CScreen, Logger):
    ...
    def read_invoice(self):
        ...
        if not address:
            self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Blackcoin address or a payment request'))
            return
        if not self.amount:
            ...
            if self.is_lightning:
                ...
                if self.payment_request:
                    ...
                    if not bitcoin.is_address(address):
                        self.app.show_error(_('Invalid Blackcoin Address') + ':\n' + address)
                        return
