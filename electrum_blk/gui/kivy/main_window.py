...
from electrum_blk.invoices import PR_PAID, PR_FAILED, Invoice
...
class ElectrumWindow(App, Logger, EventListener):
    ...
    def on_start(self):
        ...
        # bind intent for blackcoin: URI scheme
        if platform == 'android':
            from android import activity
            from jnius import autoclass
            PythonActivity = autoclass('org.kivy.android.PythonActivity')
    ...
    def notify(self, message):
        try:
            ...
            icon = (os.path.dirname(os.path.realpath(__file__))
                    + '/../../' + self.icon)
            notification.notify('Electrum-BLK', message,
                            app_icon=icon, app_name='Electrum-BLK')
        except ImportError:
