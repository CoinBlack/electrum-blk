...
if [ ! -f "$PROJECT_ROOT"/electrum_blk/libsecp256k1.0.dylib ]; then
    info "Building libsecp256k1 dylib..."
    "$CONTRIB"/make_libsecp256k1.sh || fail "Could not build libsecp"
else
    info "Skipping libsecp256k1 build: reusing already built dylib."
fi
cp "$PROJECT_ROOT"/electrum_blk/libsecp256k1.0.dylib "$CONTRIB"/osx

if [ ! -f "$PROJECT_ROOT"/electrum_blk/libzbar.0.dylib ]; then
...
cp "$PROJECT_ROOT"/electrum_blk/libzbar.0.dylib "$CONTRIB"/osx

if [ ! -f "$PROJECT_ROOT"/electrum_blk/libusb-1.0.dylib ]; then
...
cp "$PROJECT_ROOT"/electrum_blk/libusb-1.0.dylib "$CONTRIB"/osx
...
DoCodeSignMaybe ".DMG" "dist/electrum-blk-${VERSION}.dmg"
